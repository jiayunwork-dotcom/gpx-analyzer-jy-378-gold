...
func TestParseTrackHeaderOnlyEmptySlice(t *testing.T) {
	pts, err := ParseTrack(strings.NewReader("lat,lon,ele,time\n"))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if pts == nil {
		t.Fatal("ParseTrack(header only) = nil, want empty slice")
	}
	if len(pts) != 0 {
